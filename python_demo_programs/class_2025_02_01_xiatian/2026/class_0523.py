...
def relocate(dot):
    dot['x'] = random.randint(target_radius, WIDTH - target_radius)
    dot['y'] = random.randint(target_radius + 70, HEIGHT - target_radius)
    dot['spawn_time'] = pygame.time.get_ticks()
...
while running:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if not playing and button_rect.collidepoint(event.pos):
                ...
                dots = [make_dot() for _ in range(5)]
            elif playing:
                for dot in dots:
                    dx = mx - dot["x"]
                    dy = my - dot["y"]
    ...
    if playing:
        ...
        # Auto-relocate any dot that has been sitting too long
        for dot in dots:
            if now - dot["spawn_time"] >= DOT_LIFETIME:
                relocate(dot)
    ...
    if playing:
        ...
        # draw target dot
        for dot in dots:
            pygame.draw.circle(screen, RED, (dot['x'], dot['y']), target_radius)
            pygame.draw.circle(screen, BLACK, (dot['x'], dot['y']), target_radius, 3)
    else:
        if game_over:
